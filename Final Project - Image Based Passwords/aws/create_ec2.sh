...
echo "Running create_ec2.sh at ${NOW}" | tee -a ${LOGFILE}
...
aws ec2 authorize-security-group-ingress ${PREAMBLE} --group-id ${SEC_GROUP_ID} --protocol tcp --port 3000 --cidr 0.0.0.0/0 | tee -a ${LOGFILE}
aws ec2 authorize-security-group-ingress ${PREAMBLE} --group-id ${SEC_GROUP_ID} --protocol tcp --port 5000 --cidr 0.0.0.0/0 | tee -a ${LOGFILE}
...
echo ${INSTANCES_IPS} | tee -a ${LOGFILE}
